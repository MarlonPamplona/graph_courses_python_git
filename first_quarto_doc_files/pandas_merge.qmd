...
```{python}
# on argument
# this argument defines the key to join by
...
```{python}
pd.merge(tb_2019_africa,health_exp_2019) #default in inner join
```
...
## inner join
It keeps rows that are matched between the two data frames
...
Products common: 7
Product with highest ratio of positive reviews to unit sold = Desktop
