...
```{python}
# Create summary table for age groups
delay_table = tb.groupby('first_visit_type').agg(delays=('treatment_initiation_delay_(in_days)','mean')).sort_values(by='delays',ascending=False).reset_index()
...
fig3 = px.bar(
    ...
    x='first_visit_type', 
    y='delays', 
    title='Average Delays in Treatment(in Days) between Public and Private Facilities',
    labels={'first_visit_type': 'Type of Facility', 'delays': 'Delays in Days'},
    text_auto=True
)
...
Based on the chart above, the average delay in treatement in public facilities is longer than the private facilities
